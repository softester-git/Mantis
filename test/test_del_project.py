...
def test_del_project(app, db, config):
    ...
    old_projects = app.soap.get_projects(config["mantis"]["username"], config["mantis"]["password"])
    project = random.choice(old_projects)
    app.project.delete(project.id)
    assert len(old_projects) - 1 == db.project_count()
    new_projects = app.soap.get_projects(config["mantis"]["username"], config["mantis"]["password"])
